...
REASONING = os.getenv("CODEX_REASONING", "medium")  # low / medium / high / xhigh


async def call(prompt: str, *, model: str = "", reasoning: str = "") -> str:
    require_cli(CLI)

    effective_model = model or MODEL
    effective_reasoning = reasoning or REASONING
    ...
    try:
        result = await spawn(CLI, args, stdin=prompt, timeout=TIMEOUT)
    finally:
        # 读取并清理临时文件
        content = ""
        try:
